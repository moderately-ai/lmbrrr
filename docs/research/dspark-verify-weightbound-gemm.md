...
**Consequences for the ranked paths:** lever (1) hoist-the-index is dead (≤7% bundled with the whole fold); lever (2) `tensor_blockwise` hardware block-scaling is *bounded* by probe_fullk at **≤ ~15–18%** on mm2d-routed shapes (≈ +10–12% end-to-end tok/s at the margin-3.0 operating point) — and the MSL 4.1 spec (§2.22.2.2, checked 2026-07-17 against the SDK header static_asserts) prices it: scales must be **`metal_fp8_ue8m0_format` — powers of two only** — in per-**32**-along-K blocks (finer than our per-128: exact ×4 replication, fine), and a scaled B must be transposed. Since the K-loop exists ONLY to apply per-block scales, hardware scaling buys the full probe_fullk shape — but requires **requantizing the model to power-of-two d**, and decode (mv on Q2_0 blocks) and verify (planes) must use the SAME requantized weights or greedy/verify logits diverge and the byte-match invariant breaks. So lever 2 = whole-model ue8m0-constrained requant + PPL gate, for +10–12% tok/s. Cheaper exact step: B as `int2b_format` ({−1,0,+1} directly) deletes the P−rowsum machinery (rs_tg staging + barrier + one fma) while keeping the f16 per-128 software fold — but that whole family is bounded at ≤7% by probe_nofold. Beyond these, only structure-changing routes remain on M3: the B3 bitplane/popcount reformulation (ternary dot = two masked adds, abandoning the MAC/MMA structure — the one software family not covered by the refuted list) and the M5 tensor unit (D1).
