...
2. SWEEP: for each prompt length run `./target/release/lmbrrr bench --quantized-manifest target/minicpm-v46-q4k-full-text/manifest.json --quantize-lm-head q4k --warmup 1 --iterations 3 --profile medium` with the prompt override flag (check bench's prompt arg in src/main.rs; if bench can't take a prompt file, add --prompt-file — small change, gate with evals/run_gate_battery.sh). Record: prefill tok/s, TTFT, steady decode tok/s.
...
4. SPEC ARM: repeat step 2 at 128 and 4k with --drafter target/dspark-drafter-round4 — does speculation's advantage hold at depth (verify chunks touch attention KV too)?
