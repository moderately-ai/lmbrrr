...
Fused rms_norm landed (commit 450cbc1): candle_nn::ops::rms_norm with dtype-matched weight pair + LMBRRR_UNFUSED_RMSNORM=1 reference path. SDPA-vector landed (commit 1947713): l==1 unmasked decode routes through candle_nn::ops::sdpa — native GQA, strided cache k/v, no repeat_kv/k_t copies; LMBRRR_UNFUSED_SDPA=1 reference path. All gates green on both (tests, fixture, state-integrity oracle both prompts; math trajectory deviation crept 0.375 -> 0.5625 with SDPA's accumulation order, still under the 0.75 bound — watch this as numerics changes stack). Rotated 3-arm A/B, 512-token runs, 8-min cool-down (still warm-regime, 45-58 tok/s): rms_norm +1.75 tok/s paired median (~+3.5%, 5/6 rounds positive); SDPA +8.3 tok/s over baseline (~+15-17%, all rounds positive after discarding its round-1 first-run pipeline-compilation outlier). SDPA's win grows with context (old path copied the whole cache per token). Remaining pass-3 items: rope cat-branch cleanup (moot for partial_rotary=0.25? verify), MixedLinear F32 round-trip (blocked on bf16-activation-quantized-matmul-metal), cool-machine absolute numbers for the campaign doc.
