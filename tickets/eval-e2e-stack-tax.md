...
1. OURS: `./target/release/lmbrrr bench --quantized-manifest target/minicpm-v46-q4k-full-text/manifest.json --quantize-lm-head q4k --warmup 1 --iterations 3 --profile medium` -> median steady_state_tokens_per_second. Weight bytes: sum the tensor byte sizes in the manifest (python one-liner over manifest.json; record the number — approx 2.4 GB expected, verify). Effective GB/s = bytes x tok/s / 1e9.
