...
RANKED ACTIONABLE: (1) A1 finish width-7 (unblock prepare_cache OOM); (2) C1 Sequoia-DP'd depth-fill + 1 confidence-placed branch (the CORRECT tree the w=3 test got wrong); (3) B7+C2 Trees-from-Marginals rollback-free solve + lossless wide tree (top lossless lever, wants width-7 first); (4) A2 EAGLE-3 drafter (only true ceiling-raiser). Escape hatch if bounded drift OK: C4 / more relaxed acceptance. Full synthesis + arXiv refs in each child ticket; measured negatives kept as closed(wontdo) records.
