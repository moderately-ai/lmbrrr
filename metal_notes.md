...
- **M3 reality (2026-07-16, Xcode 26.6):** macOS 27.0 beta, SDK MacOSX26.5, Metal toolchain `32023.883` (MetalToolchain-v17.6.109 == the `17F109` asset). Two independent confirmations that 2-bit is not reachable today: (a) `-std=metal4.0` — `uint4b_format` compiles (control OK) but **`uint2b_format` fails** ("unknown type name; did you mean 'uint4b_format'"; only the 4-bit formats are in this build's `metal_packed_numeric` lines 15/20); (b) **`-std=metal4.1` is itself rejected** — `error: invalid value 'metal4.1' in '-std='` — the compiler has no Metal 4.1 language mode. And `xcodebuild -downloadComponent MetalToolchain` re-downloads the SAME `17F109`/`32023.883` (no newer toolchain offered). So native 2-bit `matmul2d` is **gated on a future Xcode/Metal-4.1 release**, not one download away. Re-test cheaply on any Xcode/toolchain bump: `xcrun metal -std=metal4.1 -c` a `tensor<device uint2b_format,…>` kernel — if the std is accepted and it compiles, the DSpark verify GEMM becomes a near-mechanical q4_K-mm2d mirror (docs/research/dspark-verify-weightbound-gemm.md).
