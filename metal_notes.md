...
- **M3 reality (2026-07-16):** macOS 27.0 beta, SDK MacOSX26.5, Metal toolchain `32023.883` (MetalToolchain-v17.6.109). `-std=metal4.0` compiles `uint4b_format` (control OK) but **`uint2b_format` fails** — "unknown type name; did you mean 'uint4b_format'". Only the 4-bit formats are in this build's `metal_packed_numeric` (lines 15/20). So 2-bit `matmul2d` is **spec'd but not landed in this toolchain** — re-test after any MetalToolchain / Xcode-beta update (`xcodebuild -downloadComponent MetalToolchain`); that download is the cheap unblock for a native 2-bit ternary GEMM.
